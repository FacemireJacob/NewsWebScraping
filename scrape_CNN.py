...
def scrape_article(url):
    ...
    text = soup.find_all(class_='article__main')
    ...
    try:
        ...
        if conn.is_connected():
            ...
            for i in range (0, len(textAr)):
                #Build Query
                sqlQuery = "INSERT into " + table + " (Headline, Source, Link, Body) VALUES ('" + textAr[i][1]+"', 'CNN', '" + textAr[i][0] + "', '" + textAr[i][2] + "')"
                try:
...
print("To Import URL list, store URLs as a CSV file. Please name the file the same name as the DB table it is to be inserted to. The file should be stored in the directory: (this)\\sources\\CNN. Please enter the name of the file to import:")
